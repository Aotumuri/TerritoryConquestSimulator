document.addEventListener('DOMContentLoaded', () => {
    ...
    const showBordersOnlyToggle = document.getElementById('showBordersOnlyToggle');
    const checkIsolationButton = document.getElementById('checkIsolationButton');
    ...
    absorptionPatternSelect.addEventListener('change', () => {
        checkIsolationButton.style.display = absorptionPatternSelect.value === 'modified-weighted' ? 'block' : 'none';
    });

    let absorptionCount = 0; // 吸収処理の回数をカウント
    // 孤立フラグチェック関数（指定回数に達したときにチェック）
    function checkIsolationOnFrequency() {
        // 孤立チェックが有効で、指定回数に達した場合にのみ実行
        const frequency = parseInt(isolationCheckFrequency.value, 10);
        if (enableIsolationCheck.checked && absorptionCount % frequency === 0) {
            cells.forEach(cell => {
                cell.isIsolated = isIsolatedCell(cell); // 孤立フラグを設定
            });
            drawCells(); // 再描画して孤立セルを視覚化
        }
        absorptionCount++; // 吸収処理の回数をカウント
    }
    ...
    function generateVoronoiCells(numCells, width, height) {
        ...
        return points.map((_, i) => {
            ...
            console.log(`Color:`, getRandomColor()); // 仮でランダム色
            console.log(`Neighbors of Cell ${i}:`, neighbors); // 隣接情報の確
            return cell ? {
    ...
    // 自分と同じ色の都市または首都と連結しているか判定する関数
    function isConnectedToAnyCityOrCapital(targetColor) {
        const visited = new Set();

        // 初期キューにはセルを追加
        const queue = cells.filter(cell => cell.color === targetColor);
        ...
        while (queue.length > 0) {
            ...
            // 都市または首都セルがあれば接続しているとみなす
            if (cities.includes(current) || capitals.has(current)) {
                return true;
            }
            ...
            current.neighbors.forEach(neighborIndex => {
                ...
                if (neighbor && neighbor.color === targetColor && !visited.has(neighbor)) {
                    queue.push(neighbor);
                }
            });
        }

        return false; // 都市や首都に接続されていない場合
    }
    ...
    function selectColorByPattern(neighborColors, pattern, selfColor) {
        ...
        if (pattern === 'modified-weighted') {
            ...
            // 孤立セルは吸収されやすくする
            if (cells.some(c => c.isIsolated && c.color === selfColor)) {
                const isolationFactor = 0.3; // 孤立セルの吸収率調整（例: 孤立セルは他の色に吸収されやすい）
                return Math.random() < isolationFactor ? dominantColor : selfColor;
            }

            // 通常の重み付き吸収
            return Math.random() < selfColorCount / totalWeight ? selfColor : dominantColor;
        }
        // 通常の重みつき吸収パターン
        if (pattern === 'weighted') {
            const selfColorCount = colorCounts[selfColor] || 0;
...
function buildColorGroups() {
    ...
    cells.forEach(cell => {
        ...
        // すでに探索済みのセルはスキップ
        if (colorGroups.get(color).some(group => group.has(cell))) return;

        // 新しいグループを探索
        const group = new Set();
        const hasCityOrCapital = { value: false }; // 参照型で保持

        exploreGroup(cell, group, hasCityOrCapital);

        // グループを保存
        colorGroups.get(color).push({
            cells: group,
            isConnectedToCityOrCapital: hasCityOrCapital.value
        });
    });
}

// 深さ優先探索で同色の連結領域を探索し、都市・首都と接続しているか判定
function exploreGroup(cell, group, hasCityOrCapital) {
    const stack = [cell];

    while (stack.length > 0) {
        const currentCell = stack.pop();
        if (group.has(currentCell)) continue;

        group.add(currentCell);

        // 都市または首都があればフラグを立てる
        if (capitals.has(currentCell) || cities.includes(currentCell)) {
            hasCityOrCapital.value = true;
        }

        // 隣接セルで同じ色のセルを探索
        currentCell.neighbors.forEach(neighborIndex => {
            const neighbor = cells[neighborIndex];
            if (neighbor && neighbor.color === cell.color && !group.has(neighbor)) {
                stack.push(neighbor);
            }
        });
    }
...
// 色ごとの連結性をキャッシュするマップ
let connectivityCache = new Map();

function buildConnectivityCache() {
    connectivityCache.clear();

    cells.forEach(cell => {
        // すでにキャッシュにある場合はスキップ
        if (connectivityCache.has(cell)) return;

        const color = cell.color;
        const visited = new Set();
        let isConnectedToCityOrCapital = false;

        // DFSで同色の連結成分を探索
        function exploreGroup(cell) {
            const stack = [cell];
            while (stack.length > 0) {
                const currentCell = stack.pop();
                ...
                // 都市または首都が含まれていれば孤立でない
                if (capitals.has(currentCell) || cities.includes(currentCell)) {
                    isConnectedToCityOrCapital = true;
                }

                // 隣接セルで同じ色のセルをスタックに追加
                currentCell.neighbors.forEach(neighborIndex => {
                    const neighborCell = cells[neighborIndex];
                    if (neighborCell && neighborCell.color === color && !visited.has(neighborCell)) {
        ...
        // 同色の領域をDFSで探索して都市・首都の接続状況を確認
        exploreGroup(cell);

        // 探索済みのセルに対してキャッシュを作成
        visited.forEach(visitedCell => {
            connectivityCache.set(visitedCell, isConnectedToCityOrCapital);
        });
    });
}

// 孤立セルかどうかを判定する関数
function isIsolatedCell(cell) {
    const selfColor = cell.color;
    const stack = [cell];
    const visited = new Set();
    let isConnectedToCityOrCapital = false;
    ...
    while (stack.length > 0) {
        ...
        if (visited.has(current) || current.color !== selfColor) continue;
        visited.add(current);
        ...
        current.neighbors.forEach(neighborIndex => {
            const neighbor = cells[neighborIndex];
            if (neighbor && neighbor.color === selfColor && !visited.has(neighbor)) {
                stack.push(neighbor);
            }
        });
    }

    return !isConnectedToCityOrCapital; // 首都や都市と接続されていなければ孤立
}
// 地図生成後や再生成後にグループを再構築
    // drawCells関数の更新
    function drawCells() {
    ...
    cells.forEach(cell => {
        ...
        // 孤立セルの視覚確認：孤立なら黄色で囲む
        if (isIsolatedCell(cell)) {
            ctx.lineWidth = 3;
            ctx.strokeStyle = 'yellow';
        } else if (capitals.has(cell) && capitals.get(cell) === cell.color) {
            ctx.lineWidth = 3;
            ctx.strokeStyle = 'red';
