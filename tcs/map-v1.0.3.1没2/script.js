document.addEventListener('DOMContentLoaded', () => {
    ...
    let showBordersOnly = true; // 初期設定はオン！
    ...
    function mergeAdjacentCells() {
        ...
        // 通常のセル吸収処理
        cells.forEach((cell, index) => {
            const newColor = selectColorByPattern(
                cell.neighbors.map(neighborIndex => cells[neighborIndex].color),
                'modified-weighted', // 吸収パターン
                cell.color
            );
    
            // 色が変わった場合に colorRanking を更新
            if (cell.color !== newColor) {
                updateColorRanking(cell.color, newColor);
                cell.color = newColor;
            }
    ...
    function calculateTotalColorCounts() {
        const totalColorCounts = {};
        cells.forEach(cell => {
            if (!totalColorCounts[cell.color]) {
                totalColorCounts[cell.color] = 0;
            }
            totalColorCounts[cell.color]++;
        });
        return totalColorCounts;
    }
    ...
    function selectColorByPattern(neighborColors, pattern, selfColor) {
        const totalColorCounts = calculateTotalColorCounts(); // 地図全体の色の分布を取得
        ...
        if (pattern === 'modified-weighted') {
            ...
            // `colorRanking` に基づき全体で dominantColor が多ければ10%バフ
            const selfTotalCount = colorRanking[selfColor] || 0;
            const dominantTotalCount = colorRanking[dominantColor] || 0;
    
            let modifiedSelfChance = selfColorCount / totalWeight;
            let modifiedDominantChance = dominantColorCount / totalWeight;
    
            if (dominantTotalCount > selfTotalCount) {
                modifiedDominantChance += 0.1;
                modifiedSelfChance = Math.max(0, modifiedSelfChance - 0.1);
            }
    
            if (Math.random() < modifiedSelfChance) {
                return selfColor;
            }
    ...
    // 色の分布を記録するオブジェクト
        ...
        // 初期化時に色分布を計算
        function initializeColorRanking() {
            colorRanking = {};
            cells.forEach(cell => {
                const color = cell.color;
                colorRanking[color] = (colorRanking[color] || 0) + 1;
            });
        }
        function updateColorRanking(selfColor, newColor) {
            if (selfColor !== newColor) {
                colorRanking[selfColor] = (colorRanking[selfColor] || 1) - 1;
                colorRanking[newColor] = (colorRanking[newColor] || 0) + 1;
                
                // 色が0になった場合にエントリを削除
                if (colorRanking[selfColor] === 0) {
                    delete colorRanking[selfColor];
                }
            }
        }
