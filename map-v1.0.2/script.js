document.addEventListener('DOMContentLoaded', () => {
    ...
    window.applyPreset = (presetName) => {
        if (presetName === 'random') {
            ...
            document.getElementById('cityRequirement').value = 50;
            document.getElementById('randomAbsorptionToggle').checked = true;
            document.getElementById('absorptionPattern').value = 'weighted';
        } else {
            // 他のプリセットを設定
            const preset = presets[presetName];
            document.getElementById('numCells').value = preset.numCells;
    ...
    function generateCities() {
        ...
        colorGroups.forEach((cellsOfColor, color) => {
            ...
            while (existingCitiesInColor.length < requiredCities) {
                ...
                // ランダムに選んだセルが既に都市や首都でない場合のみ追加
                if (!cities.includes(randomCell) && !capitals.has(randomCell)) {
                    cities.push(randomCell); // 新しい都市を追加
                    existingCitiesInColor.push(randomCell); // 更新したリストに追加
                }
            }
        });
    }
